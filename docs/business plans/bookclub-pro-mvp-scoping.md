...
**Podejście deweloperskie (ważne):** Pierwotny plan MVP zakłada podejście "frontend‑first" z początkowym mockowaniem backendu. Na etapie walidacji implementujemy symulacje API (lokalne mocki lub przechowywanie tymczasowe w IndexDB/LocalStorage), by szybko zweryfikować hipotezy. Rzeczywisty backend (trwałe API, serwery, integracje) zostanie zbudowany w kolejnych sprintach po zebraniu wczesnego feedbacku.
